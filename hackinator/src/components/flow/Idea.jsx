...
export default function IdeaFlow() {
  ...
  return (
    ...
    >
      ...
      {showThemes && (
        <motion.div
        className="w-full max-w-7xl px-4 space-y-6"
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {themeOptions.map((theme, idx) => (
            <motion.div
              key={idx}
              className={`flex items-center justify-between border border-gray-300 rounded-xl p-4 transition-all duration-200 hover:shadow-lg ${
                selectedThemes.includes(theme) ? 'ring-2 ring-purple-500' : ''
              }`}
        ...
        <div className="mt-8 bg-opacity-20 bg-white rounded-lg p-6 shadow-md">
          <input
            type="text"
            placeholder="Or add your own theme..."
            value={customTheme}
            onChange={(e) => setCustomTheme(e.target.value)}
            className="w-full border-b-2 border-purple-300 bg-transparent text-lg p-2 focus:outline-none focus:border-purple-600 placeholder-gray-400"
          />
        </div>
